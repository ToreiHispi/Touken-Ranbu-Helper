define((require, exports, module) => {
  ...
  return () => {
    return {
      ...
      isEnemy: false,
      get name () {
        return _.get(TRHMasterData.getMasterData('Sword'), [this.sword_id, 'name'], '-') + (_.get(TRHMasterData.getMasterData('Sword'), [this.sword_id, 'symbol'], 0) === 2 ? '·🥝' : '')
      },
      get baseId () {
      ...
      get vfatigue () {
        ...
        if (!this.inBattle && fatigue < TRH.FATIGUE.VALUE.NORMAL && !_.isUndefined(this.recovered_at) && this.status!=3 && !this.isEnemy) {
          ...
          console.log(now,this.recovered_at)
      ...
      get fatigueFlag () {
        ...
        if (fatigue <= TRH.FATIGUE.VALUE.VERY_TIRED) {
          return 0
        } else if (fatigue <= TRH.FATIGUE.VALUE.TIRED) {
          return 1
